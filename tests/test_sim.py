...
class TestSim(unittest.TestCase):
    ...
    def test_sim1(self):
        mic_array1 = load_antenna("data/eigenmike32_cartesian.csv", mic_size=2.)
        origin = np.array([0.89, 1, 1.1])
        param_dict1 = dict(mic_array=mic_array1 + origin[np.newaxis, :], src_pos=[1, 2., 0.5],
                           room_dim=[2, 3, 1.5], fs=16000, max_order=1)
        param_dict2 = dict(mic_array=mic_array1 + origin[np.newaxis, :], src_pos=[1, 2., 0.5],
                           room_dim=[2, 3, 1.5], fs=16000, max_order=1, origin=origin)
        measurements1, N1, src1, ampl1, mic_array1 = simulate_rir(param_dict1)
        measurements2, N2, src2, ampl2, mic_array2 = simulate_rir(param_dict2)
    ...
    def test_sim2(self):
        mic_array2 = np.array([[1.1, 2.1, 1],
                               [1, 0.5, 4]])
        param_dict2 = dict(mic_array=mic_array2, src_pos=[2, 1., 0.765],
